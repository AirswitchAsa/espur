...
**Optional capability — `Typer`.** An adapter may also implement `StartTyping(ctx, threadID) (stop func())` to show a "typing…" indicator while the agent runs. The core type-asserts for it; adapters that don't implement it are skipped (no behaviour change). Contract: `StartTyping` returns a non-nil `stop` and must not block on network I/O (do the first trigger and any handshake on a goroutine); typing is best-effort and never affects the reply path; the adapter pauses the indicator while its own `Post` to that thread is in flight (the delivered message replaces the bubble) and resumes after. The shared `TypingState` + `RunTypingLoop` helpers in the `adapter` package implement the pause/keepalive coordination.
...
- **Typing indicator** (optional, adapters implementing `Typer`). Triggers the platform's typing state for the duration of an agent run, refreshing on a platform cadence and pausing while a `Post` is in flight. Discord: `ChannelTyping` (self-expires ~10s, so refresh every 8s; no explicit stop — let it lapse). WeChat/iLink: resolve a `typing_ticket` via `GetConfig` (cache per thread, valid ~24h), send `status:1` every ~5s as keepalive, and send `status:2` on stop to clear the bubble; only available on threads with a cached `context_token` (DM-only, same constraint as `Post`).
